...
apt-get -y install python3-pip libxcb-xkb-dev libfontconfig-dev libgl-dev libavcodec-dev=7:4.4.2-0ubuntu0.22.04.1 libavformat-dev libavutil-dev libswscale-dev libswresample-dev libavfilter-dev
apt-get -y install meson ninja-build libxml2 libxml2-dev libxkbcommon-dev libxkbcommon0 xkb-data libxxf86vm-dev libdrm-dev libdbus-1-dev
...
if [ -d /usr/lib/x86_64-linux-gnu ]; then wget -O wine.deb https://dl.winehq.org/wine-builds/ubuntu/dists/oracular/main/binary-amd64/wine-stable_10.0.0.0~oracular-1_amd64.deb; fi
